...
const sendOrderNotifications = async (orderDetails) => {
  try {
    ...
    if (!emailUser || !emailPass) {
      ...
      console.log("📧 Order notification (console only):");
      console.log(`Order #${orderDetails.orderId} - ${orderDetails.customerName} - ₹${orderDetails.totalAmount}`);
    ...
    if (orderDetails.customerEmail) {
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              ...
              <p style="margin-top: 10px;">📞 For any queries, contact us at ${emailUser}</p>
              <p style="margin-top: 10px; font-size: 12px;">
      ...
      console.log("✅ Customer email sent to:", orderDetails.customerEmail);
    }
...
// Send delivery status update email to customer only
const sendDeliveryStatusEmail = async (orderDetails) => {
  try {
    const emailUser = process.env.EMAIL_USER;
    const emailPass = process.env.EMAIL_PASS;

    if (!emailUser || !emailPass || !orderDetails.customerEmail) {
      console.log("Email credentials or customer email not available");
    ...
    console.log(`✅ ${config.title} email sent to:`, orderDetails.customerEmail);
    return { success: true };
